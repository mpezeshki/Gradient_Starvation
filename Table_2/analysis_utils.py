...
def plot_train_val_losses(ax, train_df, val_df, y_cols, color, title, x_column=None, x_cumsum=False,
    plot_train=True, plot_val=True, smooth_val_window=1):
    ...
    val_df_copy[y_cols[0]] = val_df_copy[y_cols[0]].rolling(window=smooth_val_window).mean()
    df = train_df.merge(val_df_copy, on='epoch', suffixes=['_train','_val'])
