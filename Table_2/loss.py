...
class LossComputer:
    def __init__(self, criterion, is_robust, dataset, alpha=None, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01, normalize_loss=False, btl=False, sp=0.0, mode='', ratio=0.0):
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False):
        ...
        if self.mode == 'urw':
            actual_loss = per_sample_losses[torch.where(y == 1)].mean() + per_sample_losses[torch.where(y == 0)].mean()
            actual_loss += self.sp * (yhat[:, 0] ** 2).mean()
        elif self.mode == 'spw1':
            actual_loss += self.sp * ((yhat[:, 0] * (y * (self.ratio - 1.0) + 1.0)) ** 2).mean()
        elif self.mode == 'spw2':
            actual_loss += self.sp * ((yhat[:, 0] * ((1-y) * (self.ratio - 1.0) + 1.0)) ** 2).mean()
        elif self.mode == 'spw3':
            actual_loss += self.sp * ((yhat[torch.where(y == 1)] - 2.5) ** 2).mean()
            actual_loss += self.sp * ((yhat[torch.where(y == 0)] - 0.44) ** 2).mean()

            # actual_loss += self.sp * ((yhat[torch.where(y == 1)] - 2.5) ** 2).sum() / 
            # import pdb; pdb.set_trace()
